...
async def crawl_single_query(query: str, title: str, proxy_strategy: RoundRobinProxyStrategy):
...
async def crawl_google_ai_mode_parallel(queries: list, query_titles: list):
    ...
    tasks = [crawl_single_query(query, title, proxy_strategy) for query, title in zip(queries, query_titles)]
